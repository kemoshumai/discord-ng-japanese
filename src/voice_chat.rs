...
pub async fn join(ctx: &mut SlashContext<Arc<Context>>) -> DefaultCommandResult {
    ...
    let channel_id: NonZeroU64 = std::env::var("VOICE_CHANNEL_ID")
        ...
        .parse()
        .expect("Voice channel ID is not a number");
                ...
                for user_id in tick.silent.iter() {
                    ...
                    if wav_by_user.contains_key(user_id) {
                        ...
                        tokio::spawn(async move {
                            ...
                            // 音声認識
                            tokio::spawn(async move {
                                let recognized_text = match speech_to_text(&wav_mono).await {
                                    Ok(text) => text,
                                    Err(e) => {
                                        tracing::error!("Speech to text failed: {:?}", e);
                                        return;
                                    }
                                };

                                tracing::info!("User {}: {}", user_id, recognized_text);

                                let voice_chat_model = std::env::var("VOICE_CHAT_MODEL")
                                    .expect("Expected a voice chat model in the environment");

                                tracing::info!(
                                    "Preparing LLM request with model: {}",
                                    voice_chat_model
                                );
                                ...
                                let response = {
                                        ...
                                        let history = assistant_history.clone();
                                        tracing::debug!("History length: {}", history.len());
                                        history
                                    }
                                    .get_with_system("かよわい女の子のような口調で返信してください。女の子の名前はミーシェです。女の子はご主人様と電話しています。電話だから、返答も短めにね。")
                                    .request(&voice_chat_model).await;

                                let response = match response {
                                    Ok(r) => {
                                        tracing::info!("LLM response received: {}", r);
                                        r
                                    }
                                    Err(e) => {
                                ...
                                let response_wav = match text_to_speech(&response).await {
                                    Ok(wav) => wav,
                                    Err(e) => {
                                        tracing::error!("Text to speech failed: {:?}", e);
                                        return;
                                    }
                                };
                                ...
                                let guild_id: NonZeroU64 = std::env::var("GUILD_ID")
                                    ...
                                    .parse()
                                    .expect("Guild ID is not a number");
